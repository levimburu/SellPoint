...
export default function Checkout() {
  ...
  if (completedSale) {
    return (
      <div style={{ maxWidth: '440px', margin: '40px auto', textAlign: 'center', padding: '20px' }}>
        <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'rgba(22,163,74,0.12)', border: '3px solid var(--color-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
        ...
        <div style={{ fontSize: '42px', fontWeight: '900', color: 'var(--color-primary)', margin: '16px 0 28px', letterSpacing: '-1px' }}>
  ...
  // ── Main Checkout ─────────────────────────────────────────
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '16px', height: 'calc(100vh - 112px)', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', overflow: 'hidden', height: '100%' }}>
        <div style={{ position: 'relative', flexShrink: 0 }}>
          ...
          <input ref={searchRef} value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search product name or scan barcode..." autoFocus
            style={{ paddingLeft: '42px', fontSize: '14px' }} />
          {search && (
            <button onClick={() => setSearch('')} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-muted)' }}>
              <X size={16} />
            </button>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', flexShrink: 0, paddingBottom: '2px' }}>
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} style={{
              ...
              color: activeCategory === cat ? 'white' : 'var(--color-muted)', cursor: 'pointer',
            }}>{cat}</button>
          ))}
        </div>
              ...
              {filtered.map(product => {
                ...
                return (
                  <button key={product.id} onClick={() => addToCart(product)} style={{
                    background: inCart ? 'rgba(26,107,47,0.08)' : 'var(--color-surface)',
                    ...
                    <div style={{ fontSize: '10px', color: 'var(--color-muted)', fontWeight: '600', textTransform: 'uppercase', marginBottom: '4px' }}>{product.category}</div>
                    <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--color-text)', marginBottom: '8px', lineHeight: '1.3' }}>{product.name}</div>
                    <div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--color-primary)' }}>KES {Number(product.price).toLocaleString()}</div>
                    <div style={{ fontSize: '11px', color: product.stock_qty <= 5 ? 'var(--color-warning)' : 'var(--color-muted)', marginTop: '4px' }}>
      ...
      {/* RIGHT: Cart panel */}
      <div style={{ display: 'flex', flexDirection: 'column', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '14px', overflow: 'hidden', height: '100%' }}>

        {/* Header */}
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--color-surface-2)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '700', fontSize: '14px' }}>
            <ShoppingCart size={16} color="var(--color-primary)" />
            Order
            {cart.length > 0 && (
              <span style={{ fontSize: '11px', background: 'var(--color-primary)', color: 'white', borderRadius: '999px', padding: '1px 8px', fontWeight: '800' }}>
                {cart.reduce((s, i) => s + i.quantity, 0)} items
              </span>
            )}
          </div>
          {cart.length > 0 && (
            <button onClick={() => setCart([])} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-danger)', fontSize: '12px', fontWeight: '500' }}>Clear</button>
          )}
        </div>

        {/* Customer */}
        <div id="customer-search-wrapper" style={{ padding: '8px 12px', borderBottom: '1px solid var(--color-border)', position: 'relative', flexShrink: 0 }}>
          <div style={{ position: 'relative' }}>
            <User size={13} style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-muted)' }} />
            <input
              ...
              style={{ paddingLeft: '30px', fontSize: '12px', padding: '7px 10px 7px 30px' }}
            />
            {selectedCustomer && (
              <button onClick={() => { setSelectedCustomer(null); setCustomerSearch('') }}
                style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-muted)' }}>
                <X size={13} />
              </button>
            )}
          </div>
          {customerDropdown && !selectedCustomer && filteredCustomers.length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: '12px', right: '12px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '10px', zIndex: 30, maxHeight: '160px', overflowY: 'auto', boxShadow: '0 8px 24px rgba(0,0,0,0.15)' }}>
              {filteredCustomers.map(c => (
                <button key={c.id} onClick={() => { setSelectedCustomer(c); setCustomerDropdown(false) }}
                  style={{ width: '100%', padding: '8px 12px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'var(--color-surface-2)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'none'}
                >
                  <span style={{ fontSize: '13px', fontWeight: '500' }}>{c.name}</span>
                  {c.phone && <span style={{ fontSize: '11px', color: 'var(--color-muted)' }}>{c.phone}</span>}
                </button>
              ))}
            </div>
          )}
        ...
        <div style={{ flex: '1 1 0', overflowY: 'auto', overflowX: 'hidden' }}>
          {cart.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--color-muted)' }}>
              <ShoppingCart size={32} style={{ marginBottom: '10px', opacity: 0.2 }} />
              <p style={{ fontSize: '13px' }}>Tap products to add</p>
            </div>
          ) : (
            cart.map(item => (
              <div key={item.id} style={{ padding: '10px 12px', borderBottom: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
                  <button onClick={() => { if (item.quantity === 1) removeFromCart(item.id); else updateQty(item.id, -1) }}
                    style={{ width: '26px', height: '26px', borderRadius: '7px', background: 'var(--color-surface-2)', border: '1px solid var(--color-border)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: item.quantity === 1 ? 'var(--color-danger)' : 'var(--color-text)' }}>
                    {item.quantity === 1 ? <Trash2 size={11} /> : <Minus size={11} />}
                  </button>
                  <button onClick={() => openNumpad(item.id, item.quantity)}
                    style={{ minWidth: '30px', height: '26px', borderRadius: '7px', background: 'var(--color-primary-glow)', border: '1px solid var(--color-primary)', cursor: 'pointer', color: 'var(--color-primary)', fontSize: '13px', fontWeight: '700', padding: '0 6px' }}>
                    {item.quantity}
                  </button>
                  <button onClick={() => updateQty(item.id, 1)}
                    style={{ width: '26px', height: '26px', borderRadius: '7px', background: 'var(--color-surface-2)', border: '1px solid var(--color-border)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-text)' }}>
                    <Plus size={11} />
                  </button>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '12px', fontWeight: '600', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.name}</div>
                  <div style={{ fontSize: '11px', color: 'var(--color-muted)' }}>KES {Number(item.unit_price).toLocaleString()} each</div>
                </div>
        ...
        <div style={{ borderTop: '1px solid var(--color-border)', padding: '8px 10px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '5px' }}>
            ...
            <button onClick={() => openNumpad('discount', discount || '0')}
              style={{ background: 'var(--color-surface-2)', border: '1px solid var(--color-border)', borderRadius: '7px', padding: '4px 10px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', color: discountAmount > 0 ? 'var(--color-danger)' : 'var(--color-muted)', minWidth: '70px', textAlign: 'right' }}>
              {discountAmount > 0 ? `-${discountAmount.toLocaleString()}` : 'Add'}
            </button>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px', padding: '6px 10px', background: 'var(--color-surface-2)', borderRadius: '8px' }}>
            <span style={{ fontSize: '14px', fontWeight: '600' }}>Total</span>
            <span style={{ fontSize: '17px', fontWeight: '900', color: 'var(--color-primary)', letterSpacing: '-0.5px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px', marginBottom: '6px' }}>
            {PAYMENT_METHODS.map(m => (
              <button key={m.id} onClick={() => setPaymentMethod(m.id)} style={{
                padding: '5px 4px', borderRadius: '7px', fontSize: '11px', fontWeight: '600', cursor: 'pointer',
                border: `2px solid ${paymentMethod === m.id ? m.color : 'var(--color-border)'}`,
                background: paymentMethod === m.id ? `${m.color}15` : 'var(--color-surface-2)',
          ...
          {paymentMethod === 'M-Pesa' && (
            <div style={{ marginBottom: '10px', position: 'relative' }}>
              <Phone size={13} style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-muted)' }} />
          ...
          <button className="btn-primary" onClick={handleCompleteSale}
            ...
            style={{ width: '100%', justifyContent: 'center', padding: '10px', fontSize: '14px', fontWeight: '800', borderRadius: '8px' }}>
            {processing
              ...
              <div key={ri} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                {row.map(k => (
                  <button key={k} onClick={() => numpadPress(k)} style={{
                    ...
                    background: k === 'CLR' ? 'rgba(220,38,38,0.08)' : 'var(--color-surface-2)',
